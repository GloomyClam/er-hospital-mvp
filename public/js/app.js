...
const patientInputEls = document.querySelectorAll('[name="chiefComplaint"], [name="mentalStatus"], [name="systolicBp"], [name="diastolicBp"], [name="heartRate"], [name="respiratoryRate"], [name="spo2"], [name="temperature"], [name="bloodSugar"], [name="painScore"], [name="memo"]'); // (환자정보 에 관련된 공간) patientInputEls 공간할당
...
const hiddenSeriousDiseaseFields = new Set([
  'hpid',
  'dutyname',
  'dutyaddr',
  'dutytel1',
...
function renderSeriousDiseaseInfo(info) {
  const entries = Object.entries(info || {}).filter(([field, value]) => {
    const normalizedField = field.toLowerCase();
    if (hiddenSeriousDiseaseFields.has(normalizedField)) return false;
    if (value == null || typeof value === 'object') return false;
    return String(value).trim() !== '';
  });
  const detailList = entries
    .map(([field, value]) => {
      const label = seriousDiseaseFieldLabels[field.toLowerCase()] || field;
...
function renderCard(hospital, reasons = [], scoreInfo = null) { // renderCard 기능(병원하나를 카드로 만드는 함수)을 만들고 받는값은 hospital(병원정보), 이유 값(제외 이유값)은 빈 배열, 병원 점수는 널값을 저장한다(점수 계산 걀과값)
  ...
  const scoreReasonList = scoreInfo?.reasons// scoreReasonList 공간할당 = scoreInfo값이 실제로 존재하고 reasons(제외 이유)값도 안에 존재를 한다면, 꺼내와 저장한다 값이 없으면 undefined값을 저장한다(옵셔널 체이닝 ?.코드는 조건을 시행하다가 값이 없으면 안전하게 종료해라는 뜻임 )
    ?.map((reason) => { // 옵셔널체이닝 2개라서 scoreInfo와 이유가 다 있을 때 맵을 작동시키는 것.
      // 점수 이유는 "설명"과 "몇 점이 더해졌는지"를 같이 보여줘서 정렬 이유를 알 수 있게 한다.
      if (typeof reason === 'string') { // 만약 reason이 문자열일결우
        return `<li>${escapeHtml(reason)}</li>`; // 이유값을 이스케이프 html에 넣고 리스트로 나열한다 -> 해킹방지 후 리스트화
      }

      const pointText = reason.points > 0 ? `<span class="score-point">+${reason.points}</span>` : ''; // pointText 공간할당, reason.포인트가 0보다 클 경우 span 안에 값을 저장하여 화면에 띄운다 그렇지 않을경우 빈칸으로 표시한다
      return `<li>${escapeHtml(reason.label)} ${pointText}</li>`; // 리스트 내에 라벨과 그에 따른 점수를 같이 리스트에 묶어서 화면에 표ㅕ시한다
    })
  ...
  li.innerHTML = ` <!--li 안의 내용을 아래의 내용으로 교체한다 (병원 카드)-->
    ...
    ${scoreInfo ? ` <!-- scoreInfo의 내용이 있으면 아래 내용을 보여주고 아니면 아무것도 보여주지 말라-->
      <div class="resource-score">
        <strong>자원 여유 점수</strong> <!-- 해당 글자를 강조 -->
        <span>${scoreInfo.score}점</span> <!-- 점수를 병원마다 계산해서 적는다-->
        ${scoreReasonList ? `<ul>${scoreReasonList}</ul>` : ''} <!--scoreReasonList의 값이 있으면  <ul>${scoreReasonList}</ul> 목록을 보여준다, 없으면 값을 넣지 않는다-->
      </div>
    ` : ''}
    <dl>
      <dt>응급실 연락처</dt>
...
// localStorage 읽기/저장: key는 기존 값을 그대로 유지한다.
...
function applyPatientInfoToForm() {  // applyPatientInfoToForm 기능( 저장된 환자의 정보를 다시 화면에 표시하는 기능, 만약 환자 정보를 쓰다가 새로고침 되더라도 환자 체크 박스에 마지막에 저장했던 환자의 상태로 자동 체크해줌)
  ...
  patientInputEls.forEach((inputEl) => { //patientInputEls 환자 입력 정보를 하나씩 가져와서 inputEl이라고 한다
    inputEl.value = patientInfo[inputEl.name] || ''; // 현재 inputEl 이름과 같은 이름의 값을  patientInfo를 찾아서 inputEl 저장한다 값이 없으면 빈칸을 넣는다
...
function getValidPatientVital(fieldName, invalidFields) {
  return invalidFields.has(fieldName) ? '' : patientInfo[fieldName];
}
...
function resetTransferState() {
  ...
  patientInfo = { patientStatus: 'general' }; // 환자의 기본 상태를 general 로 한다
...
function addResourceScore(scoreInfo, label, count, maxCount, pointPerCount) { //addResourceScore 기능( 병원 추천 점수를 계산하는 코드 ) scoreInfo= 점수 결과를 모아놓는 객체, label= 어떤 자원인지 ex,응급실 병상, count= 실제 병상 갯수, maxCount= 점수로 인정할 최대 갯수, pointPerCount= 1개당 몇점을 줄지
  ...
  if (score > 0) {  // 만약 score가 0보다 크면
    ...
    scoreInfo.reasons.push({ // scoreInfo안에 들어있는 이유를 배열안에 추가로 밀어넣는다(사용자에게 보여줄 점수 반영 이유)
      label: `${label} ${count}석 반영`,  // label: "label, count 숫자 석 반영"
...
function calculateResourceScore(hospital) { // calculateResourceScore  기능(병원 하나의 자원을 확인해 점수를 계산하는 코드)
  ...
  scoreInfo.breakdown.resourceScore = scoreInfo.score; // 지금까지 계산해서 누적한 점수를  resourceScore 상세내역 에도 저장한다
