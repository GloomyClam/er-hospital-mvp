...
function addFreshnessScore(scoreInfo, updatedAt) {
  ...
  scoreInfo.breakdown.freshnessScore = freshness.points;
  if (freshness.points === 0) return;

  scoreInfo.score += freshness.points;
  scoreInfo.reasons.push({
...
function calculateResourceScore(hospital) { // calculateResourceScore  기능(병원 하나의 자원을 확인해 점수를 계산하는 코드)
  const scoreInfo = { //scoreInfo 공간할당
    ...
    breakdown: {  // 점수 세부 내역
      ...
      freshnessScore: 0, // 갱신시간이 없거나 오래된 경우에는 0점이다.
    },
  };
  ...
  scoreInfo.breakdown.resourceScore = scoreInfo.score; // 병상과 시설 여유로 계산한 점수만 자원 점수로 기록한다.
