...
function loadPatientInfo() {
  try {
    return JSON.parse(localStorage.getItem(PATIENT_INFO_STORAGE_KEY)) || {};
  } catch (err) {
    return {};
  }
}
...
function applyPatientInfoToForm() {
  patientStatusEls.forEach((radioEl) => {
    radioEl.checked = radioEl.value === (patientInfo.patientStatus || 'general');
  });

  patientInputEls.forEach((inputEl) => {
    inputEl.value = patientInfo[inputEl.name] || '';
  });
}

function readPatientInfoFromForm() {
  const checkedStatusEl = document.querySelector('input[name="patient-status"]:checked');
  const nextPatientInfo = {
    patientStatus: checkedStatusEl?.value || 'general',
  };

  patientInputEls.forEach((inputEl) => {
    nextPatientInfo[inputEl.name] = inputEl.value.trim();
  });
...
function renderPatientSummary() {
  const summaryItems = [
    ...
    ['혈압', patientInfo.systolicBp ? `${patientInfo.systolicBp} mmHg` : ''],
    ['심박수', patientInfo.heartRate ? `${patientInfo.heartRate}회/분` : ''],
    ['SpO2', patientInfo.spo2 ? `${patientInfo.spo2}%` : ''],
...
function getHospitalPhone(hospital) {
...
function handleContactClick(event) {
  ...
  const panelEl = actionEl.closest('[data-hospital-id]');
  const hospitalId = panelEl?.dataset.hospitalId;
  const action = actionEl.dataset.contactAction;
...
function getSelectedPatientStatus() {
  return patientInfo.patientStatus || 'general';
}
