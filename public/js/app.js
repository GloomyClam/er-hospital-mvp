...
function renderCard(hospital) { // renderCard 기능을 만들고 받는값은 hospital
  const li = document.createElement('li'); //li 값을 저장할 공간을 할당 = 문서 내에 li 요소를 추가로 넣는다 
  li.className = 'hospital-card';  // li의 클래스 이름은 hospital-card
...
async function loadHospitals() { // 비동기 기능(서버에서 받아온다) loadHospitals
  try { // 일단해봐
    ...
    listEl.replaceChildren(...hospitals.map(renderCard));  // hospital-list의 자식요소를 대체한다(api에서 받아온 병원 정보를 li 배열로 바꿔준다 ...은 배열을 펼쳐준다)
    listEl.hidden = false; // listEl 히든 속성을 자운다
    setStatus(`${region.stage1} ${region.stage2} · ${totalCount}곳`); // 받아온 메세지를 옮겨 적는다 (부산), (수영구), 전체 병원 수 (3)개
